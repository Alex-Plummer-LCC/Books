...
function Provider({ children }) {
    ...
    const fetchBooks = useCallback(async () => {
        const response = await axios.get("http://localhost:3001/books"); // Wait for a response from json-server to come back before setting the books.
    ...
    const deleteBookById = async (id) => {
        await axios.delete(`http://localhost:3001/books/${id}`); // Use similar syntax to line 24 to make a DELETE request to json-server.
